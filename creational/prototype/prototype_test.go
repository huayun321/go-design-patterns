...
func TestClone(t *testing.T) {
	...
	t.Logf("item1 value %v，addr %p, type %T\n", item1, item1, item1)
	t.Logf("whitePrototype value %v，addr %#v, type %T\n", whitePrototype, whitePrototype, whitePrototype)

	shirt1, ok := item1.(*Shirt)
	if !ok {
		t.Fatal("Type assertion for shirt1 could't be done successfully")
	}
	shirt1.SKU = "abbcc"
